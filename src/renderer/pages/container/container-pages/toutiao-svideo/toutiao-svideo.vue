<template>
  <section class="toutiao-svideo">
    <!-- 框架布局 -->
    <h3>下载单个头条小视频</h3><br />
    <el-input v-model="fileStorepath" placeholder="请输入头条小视频分享链接" disabled></el-input>
    <span class="line-box"></span>
    <el-button type="info" @click="alertFolderSelect">选择视频存放路径</el-button>
     <span class="line-box"></span>
    <el-input v-model="value" placeholder="请输入头条小视频分享链接"></el-input>
    <span class="line-box"></span>
    <el-button type="primary" @click="searchOneSToutiaoVideo">搜索小视频</el-button>
...
<script scoped>
    ...
    export default {
        ...
        methods:{
            // 根据视频分享链接下载该头条小视频
            searchOneSToutiaoVideo(){
                this.loading.status = true;
                this.videoArr = []
                this.axios.get(this.domain+'toutiao/svideo-one',{
                    params: {surl: this.value}
                }).then(res => {
                     this.loading.status = false;
                     this.videoInfo.url = res.data.info.url;
                     this.videoInfo.name = res.data.info.title;
                     this.videoArr.push(this.videoInfo)
                });
